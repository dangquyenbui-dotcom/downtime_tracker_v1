    ...
    def decorated_function(*args, **kwargs):
        if 'user' in session and 'session_id' in session:
            ...
            if not sessions_db.validate_session(session['session_id'], session['user']['username']):
                session.clear()
                flash('Your session has expired or you logged in from another location.', 'error')
        ...
        if username and password:
            ...
            if user_info:
                ...
                if existing_session:
                    ...
                    if request.form.get('force_login') == 'true':
                        ...
                        return jsonify({
                            ...
                            'message': f'You have an active session from {ip} ({time_str}). Logging in here will end that session.',
                            'last_ip': ip,
                ...
                if request.form.get('force_login'):
                    return jsonify({'success': False, 'message': 'Invalid credentials or access denied'})
                else:
...
def status():
    ...
    status_info = {
        ...
        'lines_count': 0,
        'users_today': 0,
        'active_sessions': 0  # Add active sessions count
    }
    ...
    if os.path.exists('templates/status.html'):
        ...
        <body style="font-family: Arial; padding: 20px; background: #f5f5f5;">
            <div style="max-width: 800px; margin: 0 auto; background: white; padding: 30px; border-radius: 10px;">
                <h1>System Status</h1>
                <div>Database: {'✅ Connected' if status_info['db_connected'] else '❌ Disconnected'}</div>
                <div>Active Directory: {'✅ Connected' if status_info['ad_connected'] else '❌ Disconnected'}</div>
                <div>Test Mode: {'Yes' if status_info['test_mode'] else 'No'}</div>
                <hr>
