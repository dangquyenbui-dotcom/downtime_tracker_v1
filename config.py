...
class Config:
    ...
    def validate(cls):
        ...
        if not cls.TEST_MODE:
            if not cls.AD_SERVER:
                errors.append("AD_SERVER is required")
            if not cls.AD_DOMAIN:
                errors.append("AD_DOMAIN is required")
            if not cls.AD_BASE_DN:
                errors.append("AD_BASE_DN is required")
        ...
        if not cls.DB_USE_WINDOWS_AUTH:
            if not cls.DB_USERNAME:
                errors.append("DB_USERNAME is required when not using Windows Auth")
            if not cls.DB_PASSWORD:
                errors.append("DB_PASSWORD is required when not using Windows Auth")
